...
export function PaymentCollector({ bookings }: { bookings: Booking[] }) {
  ...
  return (
    <form onSubmit={onSubmit} className="panel p-6">
      <h3 className="text-2xl font-semibold text-white">Advance payment collection</h3>
      <p className="mt-2 text-sm text-slate-300">Collect the required 15% table deposit and lock bookings automatically.</p>
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        ...
        <label>
          ...
          <input name="amount" type="number" min={1} className="field" placeholder="Enter advance amount" />
        </label>
        <label>
      ...
      <button disabled={loading} className="btn-primary mt-5">{loading ? 'Recording…' : 'Record payment'}</button>
